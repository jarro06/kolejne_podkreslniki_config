...
def job_config = [
    job: [
        name: "kolejne_podkreslniki_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'kolejne_podkreslniki_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/kolejne_podkreslniki_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.KolejnePodkreslniki.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "kolejne_podkreslniki", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("kolejne_podkreslniki_Feature_Build");
...
job.with {
    ...
    authenticationToken('sdgF9iEw5F4AbsYdr9HFTku46TRCk6LC')
